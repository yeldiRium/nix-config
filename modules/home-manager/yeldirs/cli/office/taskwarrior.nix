...
let
  cfg = config.yeldirs.cli.office.taskwarrior;
in
{
  options = {
    yeldirs.cli.office.taskwarrior.enable = lib.mkEnableOption "taskwarrior";
  };
  ...
  config = lib.mkIf cfg.enable {
    programs.taskwarrior = {
      ...
      config = {
        ...
        "report.next-all.columns" =
          "id,start.age,entry.age,wait.remaining,depends,priority,project,tags,recur,scheduled.countdown,due.relative,until.remaining,description,urgency";
        ...
        "report.next-all.labels" =
          "ID,Active,Age,Wait,Deps,P,Project,Tag,Recur,S,Due,Until,Description,Urg";
